...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPosition in self.allPositions:
            observationProb = self.getObservationProb(noisyDistance, pacmanPosition, ghostPosition, jailPosition)
            ghostPositionProb = self.beliefs[ghostPosition]
            self.beliefs[ghostPosition] = observationProb * ghostPositionProb
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos in newPosDist:
                newBeliefs[newPos] += newPosDist[newPos] * oldPosProb
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles): 
            self.particles.append(self.legalPositions[uniformIndex])
            uniformIndex = (uniformIndex + 1) % posLen
    ...
    def observeUpdate(self, observation, gameState):
        ...
        if (belief.total()==0):
            ...
            i = 0
            for pos in belief:
                times = round(belief[pos] * self.numParticles)
    ...
    def elapseTime(self, gameState):
        ...
        for position in belief:
            ...
            for newPos in newPosDist:
                newBelief[newPos] += newPosDist[newPos] * belief[position]
        newBelief.normalize()
        for i in range(self.numParticles):
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            if (not particle in distribution):
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        #raiseNotDefined()
        positions = self.legalPositions.copy()
        random.shuffle(positions)
        it = itertools.product(positions, repeat=self.numGhosts)
        i = 0
        while i<self.numParticles:
            particle = next(it, None)
            if (particle):
                all_diff = True
                last = None
                for pos in particle:
